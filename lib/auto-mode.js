...
// Pass each PR's review history; if `login` has any APPROVED/CHANGES_REQUESTED
// review newer than the last commit, drop it.
function selectPRsNeedingReview({ requestedPRs = [], reviewsByPRNum = {}, login }) {
  const out = [];
  for (const pr of requestedPRs) {
    if (!pr) continue;
    const reviews = reviewsByPRNum[pr.number] || [];
